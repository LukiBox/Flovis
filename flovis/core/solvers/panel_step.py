...
# Orientation choices for the fitted analysis frame: (chord_axis, span_axis).
# "auto" detects: span = largest extent, thickness = smallest of the rest.
ORIENTATIONS: dict[str, tuple[int, int] | None] = {
    "auto": None,
    "chord X, span Y": (0, 1),
    "chord X, span Z": (0, 2),
    "chord Y, span X": (1, 0),
...
def orient_nodes(nodes: np.ndarray, orientation: str = "auto"
                 ) -> tuple[np.ndarray, str]:
    """Permute the mesh axes into the analysis frame (x chord, y span, z up).

    CAD exports arrive in every convention (Z-up, span along X, ...); the
    planform fit and Cp mapping assume x = chordwise, y = spanwise,
    z = thickness. Returns the re-axed copy and the human-readable label of
    what was applied (useful when 'auto' decided).
    """
    if orientation not in ORIENTATIONS:
        raise ValueError(f"unknown orientation '{orientation}', "
                         f"options: {list(ORIENTATIONS)}")
    axes = ORIENTATIONS[orientation]
    if axes is None:                              # auto-detect from extents
        ext = nodes.max(0) - nodes.min(0)
        span_ax = int(np.argmax(ext))
        rest = [a for a in range(3) if a != span_ax]
        thick_ax = rest[int(np.argmin(ext[rest]))]
        chord_ax = ({0, 1, 2} - {span_ax, thick_ax}).pop()
        axes = (chord_ax, span_ax)
    chord_ax, span_ax = axes
    if chord_ax == span_ax:
        raise ValueError("chord and span axes must differ")
    thick_ax = ({0, 1, 2} - {chord_ax, span_ax}).pop()
    label = (f"chord {_AXIS_NAMES[chord_ax]}, span {_AXIS_NAMES[span_ax]}"
             f" (auto)" if ORIENTATIONS[orientation] is None
             else orientation)
    return np.ascontiguousarray(nodes[:, [chord_ax, span_ax, thick_ax]]), label
...
# ------------------------------------------- meshing in a killable subprocess

def _mesh_child(path: str, q) -> None:
    """Subprocess entry: mesh the STEP and ship the arrays back."""
    try:
        nodes, quads = load_and_mesh_step(path)
        q.put(("ok", nodes, quads))
    except Exception as e:  # noqa: BLE001
        q.put(("err", f"{type(e).__name__}: {e}", None))


_current_proc: _mp.Process | None = None


def cancel_current_mesh() -> None:
    """Kill a running mesh subprocess (the Cancel button)."""
...
def load_and_mesh_step_safe(path: str | Path, timeout_s: float = 180.0):
    """Mesh a STEP file in a separate process with a hard timeout.

    gmsh can grind for minutes (or hang outright) on pathological CAD, and
    it cannot be interrupted in-process - a stuck QThread used to freeze the
    analysis forever. A subprocess can always be terminated: on timeout or
    cancel the user gets a clear error instead of a dead button.

    Falls back to in-process meshing when multiprocessing is unavailable.
    """
    global _current_proc
    try:
        ctx = _mp.get_context("spawn")
        q = ctx.Queue()
        p = ctx.Process(target=_mesh_child, args=(str(path), q), daemon=True)
        p.start()
        _current_proc = p
        try:
            ...
            while _time.monotonic() < deadline:
                try:
                    ...
                    if not p.is_alive():      # cancelled or crashed child
                        raise RuntimeError(
                            "STEP meshing was cancelled (or the mesher "
                            "crashed before producing a result).")
            if result is None:
                p.terminate()
                raise TimeoutError(
                    f"STEP meshing exceeded {timeout_s:.0f} s and was "
                    f"stopped. The file may be extremely detailed or "
                    f"damaged - try exporting a simplified model (fewer "
                    f"parts, no internal structure).")
            status, a, b = result
            if status == "err":
                raise RuntimeError(f"STEP meshing failed: {a}")
            return a, b
        finally:
            _current_proc = None
            if p.is_alive():
        ...
        # no subprocess support (unusual): keep the old in-process path
        return load_and_mesh_step(path)
...
def analyze_step(path: str | Path, velocity: float = 15.0,
                 ...
                 timeout_s: float = 180.0) -> AnalysisResult:
    """
    Panel analysis of a STEP model. Requires gmsh.

    Meshing runs in a killable subprocess (see load_and_mesh_step_safe);
    everything downstream is `analyze_step_mesh`, which the UI also calls
    directly when the user just changes the orientation - no re-mesh needed.
    """
    ...
    nodes, quads = load_and_mesh_step_safe(path, timeout_s=timeout_s)
    return analyze_step_mesh(nodes, quads, model_name=Path(path).stem,
                             velocity=velocity, alphas=alphas, cg_x=cg_x,
                             n_chord=n_chord, n_span=n_span,
                             orientation=orientation)


def analyze_step_mesh(nodes: np.ndarray, quads: np.ndarray, model_name: str,
                      velocity: float = 15.0,
                      alphas=np.linspace(-2, 10, 7), cg_x: float | None = None,
                      n_chord: int = 20, n_span: int = 12,
                      orientation: str = "auto") -> AnalysisResult:
    """Planform fit + forces + Cp mapping on an already-meshed STEP model.

    Forces (CL/CD/Cm) come from lifting-line theory on the fitted planform;
    the surface Cp shown in the 3D view is the validated structured solution
    mapped onto the real geometry. The mesh is first re-axed into the
    analysis frame per ``orientation`` ('auto' detects span/chord/thickness
    from the extents - CAD arrives in every axis convention).
    """
    if len(quads) < 8:
        raise RuntimeError("The STEP mesh is empty or too coarse.")

    nodes, orient_label = orient_nodes(np.asarray(nodes, float), orientation)
