...
def fix_casadi_dll_loading() -> None:
    """Work around casadi's plugin loader breaking on non-ASCII paths.

    casadi loads its solver plugins (bspline interpolant, linsol, ...) with
    a narrow-string LoadLibrary, which mangles paths like
    ``C:\\Users\\Łukasz\\...`` and fails with WIN32 error 126 - so the VLM
    solver dies on machines whose user name has any non-ASCII character.
    Pre-loading every plugin DLL through ctypes (wide-char loader) puts them
    in the process first; casadi's by-name lookup then finds the already
    loaded modules and never touches the broken path. No-op off Windows,
    harmless when casadi is absent.
    """
    global _casadi_fixed
    if _casadi_fixed or os.name != "nt":
        return
    _casadi_fixed = True
    try:
        import ctypes
        ...
        import casadi
        pkg_dir = os.path.dirname(casadi.__file__)
        if pkg_dir.isascii():
            return                          # loader works fine on ASCII paths
        os.add_dll_directory(pkg_dir)
        for dll in glob.glob(os.path.join(pkg_dir, "libcasadi_*.dll")):
            try:
                ctypes.WinDLL(dll)
            except OSError:
                pass                        # optional plugin (CPLEX etc.)
    except Exception:  # noqa: BLE001 - never break startup over a workaround
        pass
