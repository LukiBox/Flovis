"""Theming: dark (default) / light with semantic color tokens.

Same approach as StructVis/SimVis: one token set per theme drives the Qt
palette, the QSS stylesheet, pyqtgraph, matplotlib and the PyVista 3D view,
so nothing is hardcoded per widget. Dark mode uses desaturated slate tones.
The choice is persisted in the Flovis settings file.
"""
...
def style_matplotlib() -> None:
    """Point matplotlib rcParams at the current theme (new figures only)."""
    try:
        ...
        mpl.rcParams.update({
            ...
            "text.color": t["fg"],
            "xtick.color": t["fg_muted"],
            "ytick.color": t["fg_muted"],
            "grid.color": t["divider"],
            "legend.facecolor": t["surface"],
            "legend.edgecolor": t["border_soft"],
            "font.size": 9,
        })
    except Exception:  # noqa: BLE001
        pass
...
def style_canvas(canvas) -> None:
    """Re-theme an existing MplCanvas (figure + axes) in place."""
    try:
        t = tokens()
        canvas.fig.set_facecolor(t["surface"])
