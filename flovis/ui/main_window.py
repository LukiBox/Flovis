...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.theme_cb.setFixedWidth(100)
    ...
    def _on_theme_change(self):
        ...
        if name and name != theme.get_theme():
            theme.set_theme(name)
            from PySide6.QtWidgets import QApplication
            theme.apply(QApplication.instance())
            # redraw themed canvases with the new palette
            self.templates_tab._refresh()
            if getattr(self.model3d_tab, "view", None) is not None:
                try:
                    self.model3d_tab.view.set_background(theme.view_bg())
